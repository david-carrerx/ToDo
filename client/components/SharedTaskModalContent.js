...
export default function SharedTaskModalContent({
  id,
  title,
  shared_with_id,
  completed,
  ...
  async function fetchInfo() {
    const response = await fetch(
      `http://192.168.100.14:8080/tasks/${id}`,
      {
        headers: {
  ...
  <Text style={[styles.description, { color: "white" }]}>
    {author?.name ?? "Unknown Author"}  {/* Uso de operador opcional */}
  </Text>
</View>
<View style={styles.participant}>
  <Text style={[styles.description, { color: "white" }]}>
    {sharedWith?.name ?? "Unknown Participant"}
  </Text>
</View>
    </View>
  );
